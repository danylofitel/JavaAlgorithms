...
public class TableOnRedBlackBST<Key extends Comparable<Key>, Value> implements Table<Key, Value> {
    ...
    public Iterable<Key> keys() {
        if (isEmpty()) return new QueueOnArray<>();
        return keys(min(), max());
    }
    ...
    public Iterable<Key> keys(Key lo, Key hi) {
        ...
        Queue<Key> queue = new QueueOnArray<>();
    ...
    public Iterable<Value> values() {
        if (isEmpty()) return new QueueOnArray<>();
        Queue<Value> values = new QueueOnArray<>();
        valuesInOrder(root, values);
        return values;
    }
    ...
    private void keys(Node x, Queue<Key> queue, Key lo, Key hi) {
        ...
        if (cmplo <= 0 && cmphi >= 0) queue.pushBack(x.key);
    ...
    // also ensures that data structure is a binary tree since order is strict
